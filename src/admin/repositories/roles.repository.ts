...
export class RolesRepository {
  ...
  async adminModuleCreateRole(roleName: string) {
    const createRole = await this.rolesRepo.save(
  ...
  async adminModuleGrantRoleUserIfHrGetRoleId() {
    const roleId = await this.rolesRepo.findOneBy({
      roleName: 'Hr' as RoleEnum,
    });
  ...
  async adminModuleGrantRoleUserIfManagerGetRoleId() {
    const roleId = await this.rolesRepo.findOneBy({
      roleName: 'Manager' as RoleEnum,
    });
    return roleId.roleId;
  }

  async adminModuleCreateUserGetDefaultRoleId() {
    const roleDefaultId = await this.rolesRepo.findOneBy({
      roleName: 'Employee' as RoleEnum,
    });
    return roleDefaultId;
  }
  ...
  async adminModuleCreateUserGetRoleId(roles: string) {
    ...
    return await this.rolesRepo
      ...
      .where('role_name IN (:...roleName)', { roleName: roleArray })
      .getRawMany();
  }

  async getRoleId(RoleName: string) {
    return (await this.rolesRepo.findOneBy({ roleName: RoleName as RoleEnum})).roleId
  }
}
